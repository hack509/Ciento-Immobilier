...
import { Menu, X, Search, Heart, Home, Building2, MapPin, Phone, ChevronDown, User } from 'lucide-react';
...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white shadow-sm border-b border-gray-100">
        ...
        <div className="flex items-center justify-between h-16 lg:h-18">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link
              ...
              className="p-2 rounded-lg text-gray-500 hover:text-primary-700 hover:bg-gray-100 transition-colors hidden sm:flex"
              title="Favoris"
            >
      ...
      <div
        className={cn(
          'lg:hidden fixed inset-0 top-16 z-40 bg-black/30 transition-opacity duration-300',
      ...
      <div
        className={cn(
          'lg:hidden fixed top-16 left-0 right-0 z-40 bg-white border-t border-gray-100 shadow-lg transition-transform duration-300 ease-out max-h-[calc(100vh-4rem)] overflow-y-auto',
          ...
          {!isAuthenticated && (
            <div className="pt-2 border-t border-gray-100 mt-2 space-y-2">
              <Link
                to="/auth/connexion"
                onClick={() => setMobileMenuOpen(false)}
                className="block text-center px-4 py-2.5 text-sm font-medium text-gray-700 border border-gray-300 rounded-lg hover:bg-gray-50"
              >
                Connexion
              </Link>
              <Link
                to="/auth/inscription"
                onClick={() => setMobileMenuOpen(false)}
                className="block text-center px-4 py-2.5 bg-secondary-500 text-white text-sm font-semibold rounded-lg hover:bg-secondary-600"
              >
                S&apos;inscrire
              </Link>
            </div>
